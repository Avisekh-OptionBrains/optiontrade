...
async function sendMessageToTelegram(botToken, channelId, messageText, retryCount = 0, useMarkdown = true) {
  ...
  try {
    ...
    if (!botToken || !channelId) {
      console.error("❌ CMI TELEGRAM - Missing credentials:", {
        hasBotToken: !!botToken,
        hasChannelId: !!channelId
      });
      return { ok: false, error: "Missing CMI Telegram credentials" };
    }

    if (!messageText || typeof messageText !== 'string') {
      console.error("❌ CMI TELEGRAM - Invalid message text:", typeof messageText);
      return { ok: false, error: "Invalid message text provided" };
    }

    console.log(`🚀 CMI TELEGRAM - Sending message (attempt ${retryCount + 1}/${maxRetries + 1})`);
    ...
    const response = await axios.post(url, requestData, {
      timeout: 15000, // Standardized to 15 seconds (was 20)
      headers: {
        "Content-Type": "application/json",
        "User-Agent": "CMI-Bot/1.0"
      },
    ...
    console.log("✅ CMI TELEGRAM - API response received:");
    ...
    if (response.status >= 400) {
      ...
      console.error(`❌ CMI TELEGRAM - API error: ${errorMsg}`);
      ...
      if (response.status === 400 && useMarkdown &&
        ...
        console.log(`🔄 CMI TELEGRAM - MarkdownV2 parsing failed, retrying with plain text...`);
        return sendMessageToTelegram(botToken, channelId, messageText, retryCount, false);
      }

      // Retry on server errors (5xx) but not client errors (4xx)
      if (response.status >= 500 && retryCount < maxRetries) {
        console.log(`🔄 CMI TELEGRAM - Retrying due to server error in ${retryDelay * (retryCount + 1)}ms...`);
      ...
      return {
        ok: false,
        error: `CMI Telegram API error: ${errorMsg}`,
    ...
    if (!response.data.ok) {
      ...
      console.error(`❌ CMI TELEGRAM - API returned ok: false - ${errorMsg}`);
      return {
        ok: false,
        error: `CMI Telegram API error: ${errorMsg}`,
        details: response.data
    ...
    console.error(`❌ CMI TELEGRAM - Request failed (attempt ${retryCount + 1}):`, {
    ...
    if (error.code === "ECONNABORTED" || error.code === "ETIMEDOUT") {
      if (retryCount < maxRetries) {
        console.log(`🔄 CMI TELEGRAM - Retrying due to timeout in ${retryDelay * (retryCount + 1)}ms...`);
        await new Promise(resolve => setTimeout(resolve, retryDelay * (retryCount + 1)));
        return sendMessageToTelegram(botToken, channelId, messageText, retryCount + 1, useMarkdown);
      }
      return {
        ok: false,
        error: `CMI request timed out after ${maxRetries + 1} attempts`,
    ...
    if (error.code === "ENOTFOUND" || error.code === "ECONNREFUSED") {
      return {
        ok: false,
        error: "CMI network connection failed - check internet connectivity",
        details: error.message,
      };
    }
    ...
    if (retryCount < maxRetries && (error.code === "ECONNRESET" || error.code === "EPIPE")) {
      console.log(`🔄 CMI TELEGRAM - Retrying due to network error in ${retryDelay * (retryCount + 1)}ms...`);
      await new Promise(resolve => setTimeout(resolve, retryDelay * (retryCount + 1)));
      return sendMessageToTelegram(botToken, channelId, messageText, retryCount + 1, useMarkdown);
    }
...
function CmiparseMessageText(messageText) {
  if (!messageText || typeof messageText !== "string") {
  ...
  // Updated regex to be more flexible with case and spacing for CMI strategy
  const regex = /CMI\s+(Buy|Sell)\s+(\w+)\s+at\s+(\d+(?:\.\d+)?)\s+with\s+Stop\s+Loss\s+at\s+(\d+(?:\.\d+)?)/i;
  ...
  if (match) {
    ...
    console.log("Successfully parsed CMI message:", parsed);
    return parsed;
  }
  ...
  if (altMatch) {
    ...
    console.log("Successfully parsed CMI message with alternative format:", parsed);
    return parsed;
  }

  console.error("Failed to parse CMI message:", cleanMessage);
  console.error("Expected format: 'CMI Buy/Sell SYMBOL at PRICE with Stop Loss at STOPLOSS'");
  return null;
}
